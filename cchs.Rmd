...
In Manitoba, where Indigenous peoples comprise a significant portion of the population, political organizations such as the Southern Chiefs\' Organization (SCO), Manitoba Keewatinowi Okimakanak (MKO), Assembly of Manitoba Chiefs (AMC), and Manitoba Métis Federation (MMF) have consistently advocated for research approaches that center Indigenous ways of knowing and address structural determinants of health. The Manitoba Métis Federation serves as the democratic and self-governing representative body of the Manitoba Métis Community, emphasizing nation-specific approaches to health policy that honor distinct cultural frameworks.
...
p2 <- fviz_mfa_ind(mfa_result, 
                   habillage = as.factor(analysis_data[rownames(mfa_data), "indigenous_binary"]),
                   palette = get_contrast_palette(2),
                   addEllipses = TRUE, ellipse.level = 0.95,
                   title = "B) MFA - Indigenous vs Non-Indigenous Distribution",
                   legend.title = "Indigenous Status")
...
```{r sem-diagram, echo=FALSE, fig.cap="SEM Pathway Diagram with High-Contrast Theme", out.width="90%"}
high_contrast_palette <- c(
  "#1f77b4", "#ff7f0e",  # Blue-Orange
  "#2ca02c", "#d62728",  # Green-Red
  "#9467bd", "#8c564b",  # Purple-Brown
...
# Create custom color mapping for SEM elements
sem_colors <- list(
...
# Create the SEM diagram with your theme colors
sem_diagram <- semPaths(sem_fit,
                       what = "std",
                       whatLabels = "std",
                       style = "lisrel",
                       layout = "tree2",
...
# Add title with your theme styling
title("Structural Equation Model: Healthcare Access Pathways\nStandardized Coefficients Shown", 
      line = 2, cex.main = 1.2, col = high_contrast_palette[1])

# Add legend using your theme colors
legend("bottomright", 
       ...
       cex = 0.8, bty = "n", text.col = high_contrast_palette[3])

# Save high-quality version
png("figures/sem_pathways.png", width = 12, height = 9, units = "in", res = 300)
semPaths(sem_fit,
         what = "std", whatLabels = "std",
         style = "lisrel", layout = "tree2",
         rotation = 1, curvePivot = TRUE, curve = 1.5,
         sizeMan = 10, sizeLat = 12,
         edge.label.cex = 1.3, label.cex = 1.4,
         ...
         edge.color = "black", mar = c(6,6,6,6),
         nCharNodes = 0, intercepts = FALSE, residuals = FALSE)
title("Structural Equation Model: Healthcare Access Pathways\nStandardized Coefficients Shown", 
      line = 2, cex.main = 1.2, col = high_contrast_palette[1])
legend("bottomright", 
       legend = c("Latent Construct", "Indigenous Status", "Diabetes", 
...
p_gaps_compact <- gap_analysis %>%
  ...
  scale_fill_brewer(palette = "Set2", name = "Health Indicator") +
  labs(
...
kable(gap_table, caption = "Percentage Point Gaps in Health Indicators (Indigenous - Non-Indigenous)") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
The disparity gaps analysis reveals substantial inequities across all health domains (Figure 3). First Nations populations experience `r round(gap_analysis$Unmet_Gap[1], 1)`% higher unmet healthcare needs compared to non-Indigenous populations, while Métis populations show `r round(gap_analysis$Unmet_Gap[2], 1)`% higher unmet needs. Diabetes disparities are particularly pronounced, with First Nations communities experiencing `r round(gap_analysis$Diabetes_Gap[1], 1)`% higher prevalence, contextualized within historical and intergenerational food insecurity pathways. These gaps represent not merely statistical differences but concrete manifestations of structural barriers in healthcare access and social determinants of health.
...
\setlength{\leftskip}{0.5in}
\setlength{\parskip}{8pt}
Manitoba Métis Federation. (2022). *Pulling Together: Métis Foundations Guide*.  
