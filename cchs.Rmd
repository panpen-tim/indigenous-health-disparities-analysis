...
The 2015-2016 and 2017-2018 CCHS cycles were harmonized and pooled as a cross-sectional sample rather than analyzed longitudinally for three methodological and substantive reasons:

1. **Sample Size Considerations**: Pooling cycles increased statistical power for analyzing Indigenous health disparities, particularly for subgroup analyses (First Nations, Métis, Inuit) where individual cycle sample sizes would be insufficient for robust multivariate analysis.

2. **Cross-Sectional Design of CCHS**: While CCHS collects some longitudinal data, the public use microdata files do not include person-level identifiers necessary for longitudinal linkage across cycles. The survey design treats each cycle as an independent cross-sectional sample.

3. **Policy Relevance**: Territorial organizations require contemporary, population-level estimates of structural barriers. Pooling the most recent available cycles provides the largest possible sample for identifying current disparity patterns relevant to ongoing policy advocacy.

The approach follows established methodologies for analyzing health disparities using pooled CCHS cycles (Thomas and Wannell, 2009).
